  ...
  V_pre <- data_dum %>%
    dplyr::select(
      ...
      dplyr::starts_with("q_demos_education_"),
      dplyr::starts_with("q_amazon_use_how_oft_"),
      dplyr::starts_with("q_amazon_use_hh_size_"),
  ...
  #-----------------------------------------------------------------------------
  # Chi-cuadrado / Fisher SOLO con TRAIN
  # Usamos los IDs de V_train para filtrar survey_enriched2
  #-----------------------------------------------------------------------------
  
  ids_train_cat <- V_train$response_id
  
  survey_train_cat <- survey_enriched2 %>%
    dplyr::filter(response_id %in% ids_train_cat)
  ...
  cat_survey_cols <- intersect(cat_survey_cols, names(survey_train_cat))
  
  #-----------------------------------------------------------------------------
  # Función para decidir automáticamente Chi2 o Fisher
  #-----------------------------------------------------------------------------
  
  test_cat_pair <- function(data, var1, var2, min_expected = 5) {
    
    tab <- table(data[[var1]], data[[var2]], useNA = "no")
    ...
    cramers_v <- sqrt(as.numeric(chi$statistic) / (n * (k - 1)))
  ...
  #-----------------------------------------------------------------------------
  # Aplicar Chi2/Fisher a todos los pares categóricos SOLO en TRAIN
  #-----------------------------------------------------------------------------
  
  pairs_cat <- combn(cat_survey_cols, 2, simplify = FALSE)
  
  results_chi_fisher_train <- purrr::map_dfr(
    pairs_cat,
    ~ test_cat_pair(
      data = survey_train_cat,
      var1 = .x[1],
      var2 = .x[2],
      min_expected = 5
    )
  ...
  results_chi_fisher_train <- results_chi_fisher_train %>%
    dplyr::arrange(dplyr::desc(cramers_v))
  
  print(results_chi_fisher_train)
  
  tabla_chi_fisher_latex <- results_chi_fisher_train %>%
    filter(cramers_v >= 0.10) %>%
    mutate(
      statistic = ifelse(is.na(statistic), "", round(statistic, 4)),
  ...
  latex_tabla <- kable(
    tabla_chi_fisher_latex,
    format = "latex",
    booktabs = TRUE,
    longtable = TRUE,
    caption = "Resultados de las pruebas Chi-cuadrado y Fisher entre variables categóricas en el conjunto de entrenamiento.",
    label = "tab:chi_fisher_train",
    escape = TRUE
  ) %>%
    kable_styling(
  ...
  writeLines(latex_tabla, "tabla_chi_fisher_train.tex")
