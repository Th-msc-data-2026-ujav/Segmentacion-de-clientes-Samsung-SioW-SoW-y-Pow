...
#-----------------------------------------------------------------------------
purchases <- readr::read_csv("data/survey-data/amazon-purchases.csv", show_col_types = FALSE) |>
  janitor::clean_names()
...
apple_vs_others <- period_df %>%
  dplyr::group_by(response_id) %>%
  dplyr::summarise(
    amount_apple       = sum(dplyr::if_else(tech_brand == "Apple", total_price, 0), na.rm = TRUE),
    amount_otherbrands = sum(dplyr::if_else(tech_brand != "Apple", total_price, 0), na.rm = TRUE),
...
saveWorkbook(
  wb,
  file = "data/matrices/empirical_sow.xlsx",
  overwrite = TRUE
)
...
openxlsx::saveWorkbook(wb, file = "data/matrices/matrices_data.xlsx", overwrite = TRUE)
